                    ...
                    for comp in result.chain:
                        if isinstance(comp, Plain) and len(comp.text) > 1:
                            try:
                                ...
                                # 【AI修改：强制开启双输出，确保消息链同时包含 Plain 和 Record，供 respond.stage 随机筛选】
                                dual_output = True
                                logger.debug("【AI修改】已强制 dual_output=True 以启用 respond.stage 中的随机逻辑。")
                                ...
                                if dual_output:
                                    # 如果 dual_output 为 True (即此处已被我们强制为 True)，则保留 Plain 组件
                                    new_chain.append(comp)
                            except Exception:
                                logger.error(traceback.format_exc())
