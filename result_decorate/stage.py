...
@register_stage
class ResultDecorateStage(Stage): # <-- 确保类名是 ResultDecorateStage
    ...
    async def process(
        ...
        if len(result.chain) > 0:
            ...
            # --- 【开始插入】随机选择逻辑 (30% 文本 / 70% 语音) --------------------------------
            should_generate_tts = True
            
            # 仅对 LLM 结果应用随机逻辑
            if result.is_llm_result() and self.ctx.astrbot_config["provider_tts_settings"]["enable"]:
                
                # 随机选择概率 (0.7 for text, 0.3 for voice)
                if random.random() < 0.7: 
                    should_generate_tts = False
                    logger.info("随机选择：70% 仅发送文本。跳过 TTS 生成，**节省成本**。")
                else: 
                    logger.info("随机选择：30% 仅发送语音。将进行 TTS 生成，并移除 Plain 组件。")
            
            # --- 【结束插入】随机选择逻辑 --------------------------------

            if (
                should_generate_tts  # 检查是否需要生成 TTS
                and self.ctx.astrbot_config["provider_tts_settings"]["enable"]
                and result.is_llm_result()
                    ...
                    for comp in result.chain:
                        if isinstance(comp, Plain) and len(comp.text) > 1:
                            try:
                                ...
                                # *** 只有在 70% 概率下，TTS API 才会在这里被调用和收费 ***
            ...
            # 如果随机选择跳过了 TTS (30% 概率)，则 result.chain 保持不变 (仍包含 Plain 组件)
