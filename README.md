[stage.py](https://github.com/user-attachments/files/22943279/stage.py)# Alternate-between-voice-and-text_by_qiyue
此文件用于为astrbot中的语音和文字交替输出，默认触发bot语音的概率为30%，文本为70%
实际操作非常简单，只需要替换两个文件
AstrBot-master\astrbot\core\pipeline\respond路径下替换：
[Uploimport random
import asyncio
import math
...
class RespondStage(Stage):
    ...
    async def process(
        ...
        if result.result_content_type == ResultContentType.STREAMING_RESULT:
            ...
            # --- 【开始插入】随机发送逻辑 (30% 文字 / 70% 语音) ------------------------
            
            # 1. 检查是否存在 Plain (文字) 和 Record (语音)
            has_plain = any(isinstance(comp, Comp.Plain) for comp in result.chain)
            has_record = any(isinstance(comp, Comp.Record) for comp in result.chain)
        ...
        event.clear_result()ading stage.py…]()
